...
@pytest.fixture
def instrumentation():
    inst = RuntimeInstrumentation(_Queue())
    yield inst
    try:
        inst.cleanup_hooks()
    except Exception:
...
def test_no_per_call_trace_hook_is_installed(instrumentation):
    """THE regression test. If this fails, every host application pays the per-call tax again."""
    instrumentation.setup_hooks()
    assert instrumentation._monitoring_tool_id is not None, "did not take the sys.monitoring path"
...
@pytest.mark.skipif(sys.version_info < (3, 12), reason="PEP 669 requires Python 3.12+")
def test_instrumentation_overhead_is_negligible(instrumentation):
...
@pytest.mark.skipif(sys.version_info < (3, 12), reason="PEP 669 requires Python 3.12+")
def test_exceptions_are_still_seen(instrumentation, tmp_path):
    ...
    try:
        ...
        try:
            ...
            pass
        assert instrumentation._last_captured_exception is not None, (
